...
public:
    int jobID, TTL, TLL; // Total Time Limit & Total Line Limit
    int TTC,LLC; // Total Time Counter & Line Limit Counter
};
...
private:
    fstream infile;
    fstream outfile;
    char Memory[300][4], buffer[40], IR[4], GPR[4];  
    bool toggle_register;     
    int IC; // Instruction Counter
    int SI; // System Interrupt : 1 - GD , 2 - PD , 3 - H
    int PI; // Program/Page Interrupt : 0 - Normal , 1 - Operation code error, 2 - Operand error, 3 - Page Fault
    int TI; // Timer Interrupt : 0 - Normal time , 2 - TLE 
    PCB pcb;
    int PTR; // Page Table Register -> Address at which Page Table starts
    ...
    void init(){
        ...
        PTR=PTE=pageNo=-1;
        fill(isAllocated,isAllocated + sizeof(isAllocated), 0);
        fill(TLB,TLB + sizeof(TLB), -1);
        pageTablePTR = 0;
        Terminate=false;
    }
    ...
    int Allocate(){ // Gives a random page no. b/w 0 and 29 which is not already allocated
        int pageNo;
        bool check=true;
        while(check){
            pageNo = (rand() % 30) ;
            if(isAllocated[pageNo]==0){
                isAllocated[pageNo] = 1;
                check=false;
            }
        }
        ...
        if (infile.is_open()){
            ...
            while (getline(infile, s)){
                if (s[0] == '$' && s[1] == 'A' && s[2] == 'M' && s[3] == 'J'){
                    ...
                    cout << "Program Card "<<pageTablePTR <<" loading at page "<<pageNo<<endl;
    ...
    void WRITE(){
        ...
        pcb.LLC++; // Incrementing Line Limit Count
        if(pcb.LLC>pcb.TLL){
            TERMINATE(2);
            return;
        }
    ...
    void TERMINATE(int EM){
        ...
        switch(EM){
            ...
            case 0:
                outfile<<"NO ERROR : result stored successfully\n";
                break;
            case 1:
    ...
    int ADDRESSMAP(int virtual_address){
        if (0 <= virtual_address  &&  virtual_address < 100){ // 10(size of PT) * 10(words in a line) = 100
            PTE = PTR + (virtual_address / 10);
            cout<<"PTE for virtual address "<<virtual_address<<" is "<<PTE<<endl;
            cout<<Memory[PTE][0]<<Memory[PTE][1]<<endl;
            if (Memory[PTE][0] == '*' || Memory[PTE][0] == '\0'){
                PI = 3; // Page fault
                cout<<"Page fault for virtual address "<<virtual_address<<endl;
                MOS();
            }
            else{
                string p;
            ...
            PI = 2; // Operand Error -> Memory address is not a 2 byte number
            MOS();
        }
        return pageNo * 10; // ?
    }
        ...
        while (!Terminate){   
            ...
            if (IR[0] == 'G' && IR[1] == 'D'){
                ...
                if (!isdigit(IR[2]) || !isdigit(IR[3])){
                    ...
                    virtual_address = (stoi(op))/10;
                    if(TLB[virtual_address]==-1){
                        int allocated_page=Allocate();
                        TLB[virtual_address]=allocated_page;
                        isAllocated[allocated_page]=1;
                    }
                    real_address = TLB[virtual_address];
                    cout<<"Real Address of Page "<<virtual_address<<" is Page "<<real_address<<endl;
                    SI = 1;
                    MOS();
                }
            }
            ...
            // PD - PRINT DATA  
                ...
                if (!isdigit(IR[2]) || !isdigit(IR[3])){
                    ...
                    virtual_address = (stoi(op))/10;
                    real_address = TLB[virtual_address];
                    if(real_address==-1) // page fault error
                    SI = 2;
                    MOS();
                }
            }
